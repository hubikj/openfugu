...
- **M4 — iOS BLE bring-up: core VERIFIED (2026-07-17).** Kable's CoreBluetooth backend works against real eFugu hardware on the iPhone with no code changes — permission prompt, connect, live pressure, and **two devices concurrently in a multiplayer game**. Remaining before calling M4 done: `.fugu` import (UTType registration), confirm UUID-keyed device identity persists correctly (saved devices / device-user pairings across relaunches and reinstalls), and the on-device UI findings: color pass, cramped layouts, UUID display overflow. Iteration loop is push-to-main → SideStore update (~20 min).
