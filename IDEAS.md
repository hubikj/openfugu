...
- **M4 — iOS BLE bring-up.** Kable's CoreBluetooth backend against a real eFugu: Info.plist permission strings, UUID identity, connection concurrency, `.fugu` import (UTType), portrait lock. Protocol already proven in M2; iteration loop is now push-to-main → SideStore update (~20 min). Fold in the M3 on-device UI findings: color pass, cramped layouts, UUID display overflow.
